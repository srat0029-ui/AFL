...
import { formatValue, isExcludedGame, selectEvidence, type ContextEvidenceGame, type EvidenceFilters, type EvidenceOrder } from "../features/playerContext";
...
function ExcludedGames({ rows, stat, teammateName }: { rows: ContextEvidenceGame[]; stat: string; teammateName?: string }) {
  if (rows.length === 0) return null;
  const ordered = [...rows].sort((a, b) => b.scheduled_start.localeCompare(a.scheduled_start) || b.match_id - a.match_id);
  return <details className="research-excluded-games">
    <summary>Excluded from comparison ({rows.length})</summary>
    <p className="hint">These games fall outside {teammateName ?? "the teammate"}'s comparable tenure - before they were recorded at this club, or while they were recorded at another club. They are not teammate-out evidence and are in no average, rate, sample size or confidence figure.</p>
    <div className="table-scroll" role="region" aria-label="Games excluded from the comparison" tabIndex={0}><table className="data-table research-evidence-table"><thead><tr><th scope="col">Match</th><th scope="col">Date</th><th scope="col">Opponent</th><th scope="col">{stat}</th></tr></thead>
      <tbody>{ordered.map(game => <tr key={game.match_id}><td><Link to={`/matches/${game.match_id}`}>{game.round_name ?? `Round ${game.round_number}`}, {game.season_year}</Link></td><td>{Number.isNaN(Date.parse(game.scheduled_start)) ? "Unavailable" : <time dateTime={game.scheduled_start}>{dateFormat.format(new Date(game.scheduled_start))}</time>}</td><td>{game.opponent_name ?? "Unavailable"}</td><td className="num">{formatValue(game.stat_value, 0)}</td></tr>)}</tbody></table></div>
  </details>;
}

export default function PlayerContextEvidence({ rows: allRows, stat, teammateName }: { rows: ContextEvidenceGame[]; stat: string; teammateName?: string }) {
  const [filters, setFilters] = useState<EvidenceFilters>(DEFAULT_FILTERS);
  const [page, setPage] = useState(0);
  const rows = useMemo(() => allRows.filter(row => !isExcludedGame(row)), [allRows]);
  const excludedRows = useMemo(() => allRows.filter(isExcludedGame), [allRows]);
  ...
  return <section aria-labelledby="evidence-heading">
    <div className="section-row research-evidence-heading"><div><h2 id="evidence-heading">Inspect the evidence</h2><p className="hint">Filters below affect this table only. The comparison above uses every comparison-eligible game in the selected scope.</p></div></div>
    ...
    <p className="hint" role="status">{evidence.length === 0 ? "No matching games" : `Showing ${first + 1}–${first + visible.length} of ${evidence.length} matching games`} · {rows.length} comparison games in this scope</p>
    ...
    <ExcludedGames rows={excludedRows} stat={stat} teammateName={teammateName} />
  </section>;
}
