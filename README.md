...
- Player Research teammate context is scoped to an explicit, visible time window - current season (default), last 2 seasons, or current-club career - never silently broadened when a sample is too small; the same window drives teammate discovery and the detail view. "Without" means no same-club match record, so games played while the teammate was at another club are counted as "apart" and flagged in the response rather than removed; role and opponent strength are still not controlled for.
