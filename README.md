...
- Provision the external free-tier Postgres and a real Render account from `render.yaml`, add the GitHub Actions repository secrets the two scheduled workflows need, and run `scripts/smoke_test.py` against the live deployment — the one step this phase's audit trail stops short of.
