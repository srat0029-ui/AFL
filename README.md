...
**Multi Builder selection (High Probability mode).** Selection is not "highest individual probability wins": that objective picked the easiest, shortest-priced lines and then needed many legs to reach the tier's odds band, adding failure points. It now prefers the fewest legs (caps 3/4/5/6 by tier) that reach the band; per player it offers the most demanding line that still clears the tier's probability floor (nothing is banned - a player whose only qualifying line is 10+ still gets it); and it prefers lines offered by at least half of the match's bookmakers. That last signal is a **coverage proxy** (bookmakers quoting the exact market), explicitly not popularity or liquidity, and it can only break ties - a weakest leg more than 5 percentage points stronger always wins. The UI states why a leg is there ("Offered by 5 of 7 bookmakers") and never invents a joint probability: where the validated SGM engine does not cover a combination it says so and shows only the price-implied probability. The evidence behind this, including what it does *not* show, is in [docs/MULTI_BUILDER_AUDIT.md](docs/MULTI_BUILDER_AUDIT.md) (reproducible via `backend/scripts/multi_builder_audit.py`).
