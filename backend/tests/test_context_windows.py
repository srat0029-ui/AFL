...
def test_current_club_career_filtering(db_session):
    ...
    assert (a.with_teammate.games, a.without_teammate.games) == (16, 12)
...
def test_teammate_who_joined_partway_is_windowed_consistently(db_session):
    """The teammate's games are matched by (same team, same match). Windows cut
    the player's games first, so a teammate who arrived in 2026 has NO games in
    the 2025-only part of any window. NOTE (documented limitation): games from
    before a teammate's arrival are counted as "apart" in every window, exactly
    as the pre-window behaviour did."""
    ...
    cs, l2 = analyse(w, CS), analyse(w, L2)
    assert (cs.with_teammate.games, cs.without_teammate.games) == (9, 3)
    assert (l2.with_teammate.games, l2.without_teammate.games) == (9, 11)
    assert [s.season_year for s in l2.season_breakdown] == [2026, 2025]
    old = next(s for s in l2.season_breakdown if s.season_year == 2025)
    assert old.with_teammate.games == 0 and old.with_teammate.mean is None and old.without_teammate.games == 8
...
# --- teammate arrival transparency ---------------------------------------------

def test_apart_games_before_the_teammate_joined_the_club_are_counted_and_flagged_but_not_removed(db_session):
    w = World(db_session, "arr")
    for _ in range(8):
        w.game(2025, player_disposals=20, teammate_in=False)   # teammate not at the club yet
    for _ in range(3):
        w.game(2026, player_disposals=24, teammate_in=False)   # still not at the club
    for _ in range(9):
        w.game(2026, player_disposals=27, teammate_in=True)
    for _ in range(2):
        w.game(2026, player_disposals=25, teammate_in=False)   # a genuine absence AFTER arriving
    l2 = analyse(w, L2)
    assert l2.without_teammate.games == 13                      # numbers unchanged: nothing removed
    t = l2.teammate_tenure
    assert t.apart_games == 13 and t.apart_games_not_at_club == 11
    assert "11 of the 13 games apart" in t.note and "may not be true absences" in t.note
    cs = analyse(w, CS)
    assert cs.teammate_tenure.apart_games_not_at_club == 3 and cs.teammate_tenure.apart_games == 5


def test_no_arrival_note_when_every_apart_game_followed_the_teammates_arrival(db_session):
    w = World(db_session, "noarr")
    for _ in range(9):
        w.game(2026, player_disposals=27, teammate_in=True)
    for _ in range(4):
        w.game(2026, player_disposals=25, teammate_in=False)
    t = analyse(w, CS).teammate_tenure
    assert t.apart_games_not_at_club == 0 and t.note is None


def test_api_exposes_teammate_tenure(client, db_session):
    w = build_career(db_session)
    body = client.get(f"/api/afl/players/{w.player.id}/context/{w.teammate.id}?window=current_club_career").json()
    assert body["teammate_tenure"]["apart_games_not_at_club"] == 6  # the 2023 games, before the teammate's first game
    assert body["teammate_tenure"]["note"]


def test_apart_games_while_the_teammate_was_at_another_club_are_flagged_but_real_absences_are_not(db_session):
    """Real-data shape: the teammate played here, left for another club, then
    came back. Apart games during the spell elsewhere are flagged; an apart game
    right after a game for THIS club (injury/rest) is not."""
    w = World(db_session, "spell")
    for _ in range(6):
        w.game(2024, player_disposals=20, teammate_in=True)                                   # together
    w.game(2024, player_disposals=20, teammate_in=False)                                      # real absence: last game was for this club
    for _ in range(5):
        w.game(2025, player_disposals=18, teammate_in=True, teammate_club=w.club_b)          # teammate playing for ANOTHER club
    for _ in range(4):
        w.game(2025, player_disposals=18, teammate_in=False)                                  # apart; teammate's last game was at club_b
    for _ in range(4):
        w.game(2026, player_disposals=22, teammate_in=True)                                   # back
    t = analyse(w, CAREER).teammate_tenure
    assert t.apart_games == 1 + 5 + 4
    # Flagged only when the evidence is clear. Not flagged: the 2024 absence (teammate's last game was for this
    # club) and the FIRST game after they left (still ambiguous - it follows a game for this club).
    assert t.apart_games_not_at_club == 4 + 4
    assert analyse(w, CAREER).without_teammate.games == 10  # nothing removed from the split
    assert analyse(w, CS).teammate_tenure.note is None      # 2026: teammate was back, no apart games at all
